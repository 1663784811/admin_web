<template>
      ...
      <div class="comBox">
        <div class="comItem" v-for="(item, index) in state.pageObj" :key="index" @click="clickComponentItem(item)">
          <div>组件ID:{{ item.id }} 名称:{{ item.name }} 类型:{{ item.type }}
            ...
            <Button class="dataBtn" type="warning" icon="md-create" size="small" @click="commonTableToNewTable(item)" v-if="item.type === 'commonTable'">转新表格</Button>
            <Button class="dataBtn" type="success" icon="md-create" size="small">组件日志</Button>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="configOperation" v-show="configModule.configPage.showOperation">
    <div class="closeBox">
      <Button class="dataBtn" type="error" icon="md-close-circle" size="small"
              @click="configModule.configPage.showOperation = false"/>
    </div>
...
const {winIcon, winCode, winData} = useWinModal();
...
const addPage = () => {
  winData.show = true;
  winData.url = "/admin/config/page/saveCPage";
  winData.data = {};
  winData.pageCode = ''
  initPage();
}
...
const updateComponent = (item) => {
  winData.show = true;
  initSave();
  winData.changeDataFn = changeDataFn;
  findIdCPageComponents({
    id: item.id
  }).then(rest => {
    winData.data = rest.data;
  })
}
  ...
  if (dataObj.key === 'type') {
    if (dataObj.allData) {
      if (!dataObj.allData.componentsCode) {
        winData.data.componentsCode = dataObj.value;
      }
    }
  }
}
...
const initSave = () => {
  winData.url = "/admin/config/cpagecomponents/saveCPageComponents";
  winData.data = {};
  winData.pageCode = 'sssss'
  winData.columns = [
    {
      "width": 100,
      "key": "icon",
